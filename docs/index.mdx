---
title: "kiro-generator"
description: "Manage Kiro custom agents in TOML instead of hand-editing piles of JSON."
---
...
<Note>
  This Mintlify site is being migrated from the old `mdbook` docs. The pages
  below are the curated first slice, not a full one-to-one port.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Guided migration" icon="sparkles" href="/guided-migration">
...
- [Quickstart](/quickstart) walks through the `--skeleton` path
- [Guided migration](/guided-migration) covers the `kg-helper` migration flow
