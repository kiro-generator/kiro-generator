---
title: "Templates"
description: "Use template agents as reusable building blocks without generating standalone JSON files."
---
...
[agents.git-readonly.nativeTools.shell]
allow = [
    "git status .*",
    "git fetch .*",
    "git diff .*",
...
forceAllow = ["git add .*", "git commit .*"]

[agents.git-full]
template = true
inherits = ["git-write"]

[agents.git-full.nativeTools.shell]
forceAllow = ["git push .*", "git pull .*"]
```
